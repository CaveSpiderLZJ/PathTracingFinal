...
public:
    ...
    bool intersect(const Ray &ray, Hit &hit, float tmin, float& u, float& v) override {
        ...
        if(mesh->intersect(ray, hit, tmin, u, v)){
            ...
            float theta = atan(inter[0] / inter[2]) + (inter[2] > 0.0f ? 
                (inter[0] > 0.0f ? 0.0f : 2.0f) : 1.0f) * M_PI;
            ...
            for(int i = 0; i < ITER; i++){
                ...
                Vector3f Fx = ray.origin + x[0] * dir
                    - Vector3f(sinTheta * point.V[0], point.V[1], cosTheta * point.V[0]);
                Matrix3f Fprime(dir[0], -sinTheta * point.T[0], -cosTheta * point.V[0],
                                dir[1], -point.T[1], 0.0f,
                                dir[2], -cosTheta * point.T[0], sinTheta * point.V[0]);
                Fprime.transpose();
                x = x - 0.05f * Fprime.inverse() * Fx;
            }
            Vector3f w = Vector3f::cross(Vector3f::UP, point.T);
            Vector3f normal = Vector3f::cross(point.T, w).normalized();
