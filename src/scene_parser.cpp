...
Material *SceneParser::parseMaterial() {
    ...
    while (true) {
        ...
        else if(strcmp(token, "textureLuminance") == 0)
            textureLuminance = readFloat();
        else{
            assert (!strcmp(token, "}"));
            break;
        }
    ...
    auto *answer = new Material(diffuseColor, specularColor, luminance, shininess,
        refractiveIndex, fresnel, filename, textureScale, textureLuminance);
    return answer;
}

Fresnel SceneParser::parseFresnel(){
    char token[MAX_PARSER_TOKEN_LENGTH];
    char filename[MAX_PARSER_TOKEN_LENGTH];
    filename[0] = 0;
    float fbase = 0.0f;
    ...
    while (true) {
        getToken(token);
        if (strcmp(token, "fbase") == 0) {
            fbase = readFloat();
        } else if (strcmp(token, "fscale") == 0) {
            fscale = readFloat();
    ...
    return Fresnel(fbase, fscale, power);
}
